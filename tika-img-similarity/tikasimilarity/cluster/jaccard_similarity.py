...
def computeScores(inputDir, outCSV, acceptTypes):
    ...
    with open(outCSV, openMode) as outF:
      ...
      for file1, file2 in files_tuple:
        f1MetaData = parser.from_file(file1)["content"]
        f2MetaData = parser.from_file(file2)["content"]

        f1MetaData = f1MetaData.strip()
        f1MetaData = ast.literal_eval(f1MetaData)

        f2MetaData = f2MetaData.strip()
        f2MetaData = ast.literal_eval(f2MetaData)
        print(type(f1MetaData))
        print(type(f2MetaData))
