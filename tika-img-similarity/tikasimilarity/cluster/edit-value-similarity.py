    ...
    with open(outCSV, openMode) as outF:
        ...
        for file1, file2 in files_tuple:
            try:
                ...
                file1_parsedData = file1_parsedData["content"].strip()
                file1_parsedData= ast.literal_eval(file1_parsedData)

                file1_parsedData = {k:v for k,v in file1_parsedData.items() if k != "jaccard_score"}

                file2_parsedData = file2_parsedData["content"].strip()
                file2_parsedData = ast.literal_eval(file2_parsedData)
                file2_parsedData = {k: v for k, v in file2_parsedData.items() if k != "jaccard_score"}
                ...
                for feature in intersect_features:
                    ...
                    if len(file1_feature_value) == 0 and len(file2_feature_value) == 0:
                        ...
                        feature_distance = float(editdistance.eval(file1_feature_value, file2_feature_value))/(
                            len(file1_feature_value) if len(file1_feature_value) > len(file2_feature_value) else len(file2_feature_value))
                        feature_distance = round(feature_distance, 1)
